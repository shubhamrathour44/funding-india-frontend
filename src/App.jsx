...
function App() {
  ...
  // Get live LTP for selected symbol
  const currentPrice = prices[symbol]?.ltp || entryPrice;
  const pnlColor = (pnl) => (pnl > 0 ? "#4ade80" : pnl < 0 ? "#f87171" : "#999");
    ...
    <div className="terminal">
      ...
      <div className="main-layout">
        <div className="watchlist">
          <h3>📋 Watchlist</h3>
          {["NIFTY 24200 CE", "NIFTY 24200 PE", "BANKNIFTY 52000 CE", "SENSEX 82000 PE"].map(
            (s) => (
              <div
                key={s}
                onClick={() => setSymbol(s)}
                ...
                style={{
                  ...
                  padding: "8px",
                  borderRadius: "4px",
                  backgroundColor: symbol === s ? "#333" : "transparent",
                  marginBottom: "4px",
                }}
              >
                <div>{s}</div>
        ...
        <div className="chart-area">
          ...
          <div style={{ padding: "20px", textAlign: "center", color: "#888" }}>
            <p>TradingView Chart (Optional Integration)</p>
            <p style={{ fontSize: "12px" }}>
              Embed TradingView iframe here for technical analysis
            </p>
          </div>
        </div>
        ...
        <div className="order-panel">
          ...
          <div style={{ marginBottom: "10px", padding: "10px", backgroundColor: "#1a1a1a", borderRadius: "4px" }}>
            <div style={{ fontSize: "12px", color: "#888" }}>Selected Symbol</div>
            <div style={{ fontSize: "18px", fontWeight: "bold" }}>{symbol}</div>
            <div style={{ fontSize: "14px", color: "#4ade80" }}>LTP: ₹{currentPrice}</div>
          </div>
          ...
          <input
            ...
            placeholder="Entry Price (auto-fill with LTP)"
            type="number"
            style={{ width: "100%", padding: "8px", marginBottom: "8px" }}
          />
          <input
      ...
      <div className="tables">
            ...
            <table>
              ...
              <tbody>
                {positions.map((p) => (
                  <tr key={p.order_id}>
                    <td>{p.symbol}</td>
                    <td style={{ color: p.direction === "BUY" ? "#4ade80" : "#f87171" }}>
                      {p.direction}
                    </td>
                    <td>{p.lots}</td>
                    <td>₹{p.entry_price}</td>
                    <td>₹{p.current_ltp}</td>
                    <td style={{ color: pnlColor(p.unrealised_pnl) }}>
                      {p.unrealised_pnl_fmt}
                    </td>
                    <td>
                      <button
                        onClick={() => closeOrder(p.order_id, p.current_ltp)}
                        style={{
                          padding: "4px 8px",
        ...
        <div className="table-card">
          ...
          <p style={{ color: "#888", textAlign: "center", padding: "20px" }}>
            History temporarily disabled while backend history endpoint is fixed.
          </p>
        </div>
      </div>
    </div>
